...
public class UserLocation {
    ...
    public static List<UserLocation> getAllUserLocs(Context context, long userSeq, int limit, long lastSeq) {
        ...
        try {
            ...
            cursor = db.query(UserLocation.TABLE_NAME,
    ...
    private static UserLocation cursorToUserLoc(Cursor cursor) {
        UserLocation ul = new UserLocation();
        ul.setId(cursor.getLong(0));
        ul.setUserseq(cursor.getLong(1));
        ul.setLat(cursor.getDouble(2));
        ul.setLng(cursor.getDouble(3));
        ul.setStampDate(MySQLiteHelper.ConvertStrToDate(cursor.getString(4)));
        return ul;
    }
    ...
    public synchronized void Save(Context context) {
        ...
        try {
            ...
            values.put(UserLocation.COLUMN_STAMP_DATE,
                    MySQLiteHelper.getDateTime(this.getStampDate()));

            db.insert(UserLocation.TABLE_NAME,
                    null,
                    values);
